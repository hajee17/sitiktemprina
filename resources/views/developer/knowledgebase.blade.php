...
@section('content')
<div class="w-full min-h-screen bg-[#F5F6FA] p-6"> {{-- Hapus mt-[80px] dan ml-[280px] dari sini, tambahkan p-6 --}}
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6"> {{-- Gunakan flex-col di mobile, flex-row di md --}}
        <h1 class="text-2xl font-bold text-gray-800 mb-4 md:mb-0">Knowledge Base</h1> {{-- Tambahkan margin-bottom di mobile --}}
        <div class="flex gap-3 w-full md:w-auto justify-end"> {{-- Atur lebar dan justify-end di mobile --}}
    ...
    <form action="{{ route('developer.knowledgebase.index') }}" method="GET">
        <div class="flex flex-col md:flex-row items-stretch md:items-center gap-4 mb-6"> {{-- Gunakan flex-col di mobile, items-stretch agar input penuh --}}
            ...
            <button type="submit" class="w-full md:w-auto px-4 py-2 bg-black text-white rounded-full hover:bg-gray-800">Cari</button> {{-- Atur lebar tombol --}}
        </div>
    </form>

    {{-- @todo: Implement filter logic in controller --}}
    <div class="flex flex-wrap gap-3 mb-6"> {{-- Gunakan flex-wrap agar tombol ke bawah di mobile --}}
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        @forelse($knowledgeBases as $kb)
            <div class="bg-white p-4 rounded-xl shadow-sm border flex flex-col sm:flex-row items-start justify-between"> {{-- flex-col di mobile, flex-row di sm --}}
                <div class="flex gap-4 w-full sm:w-auto"> {{-- Pastikan konten mengambil lebar penuh di mobile --}}
                    ...
                    <div class="flex-grow"> {{-- Tambahkan flex-grow untuk mengisi ruang yang tersedia --}}
                ...
                <div class="relative mt-4 sm:mt-0 sm:ml-4 flex-shrink-0"> {{-- Margin atas di mobile, margin kiri di sm, flex-shrink agar tidak mengecil --}}
